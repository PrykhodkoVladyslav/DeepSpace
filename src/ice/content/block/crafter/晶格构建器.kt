...
class 晶格构建器 :NormalCrafter("lattice_constructor") {
  init {
    localization {
      zh_CN {
        ...
        for(dist in 2 downTo 0) {
          ...
          if (e.workEfficiency() > 0) {
            ...
            for(i in 0..3) {
              Draw.rect(
                wave,
                e.x + dist * Geometry.d4(i).x * 3 + 5 * (Geometry.d4(i).x.compareTo(0)),
                e.y + dist * Geometry.d4(i).y * 3 + 5 * (Geometry.d4(i).y.compareTo(0)),
                ((i + 1) * 90).toFloat()
              )
            }
