...
class 震击 :SglTurret("turret_shock") {
  ...
  fun setAmmo() {
    ...
    addAmmoType(IItems.生物钢) {
      ...
      PointBulletType().apply pointBulletType@{
        ...
        shootEffect = Effect(24f) { e ->
          e.scaled(10f) { b ->
            Draw.color(Color.white, b1, b.fin())
            Lines.stroke(b.fout() * 3f + 0.2f)
            Lines.circle(b.x, b.y, b.fin() * 50f)
          }
          Draw.color(b1)
          ...
          Drawf.light(e.x, e.y, 180f, b1, 0.9f * e.fout())
        }
        smokeEffect = Fx.smokeCloud
        trailEffect = Effect(30f) { e: EffectContainer ->
          for(i in 0..1) {
            Draw.color(if (i == 0) b1 else IceColor.b4)
          ...
          Drawf.light(e.x, e.y, 60f, b1, 0.6f * e.fout())
        }
        trailSpacing = 20f
        hitEffect = Effect(20f, 200f) { e ->
          Draw.color(b1)
        ...
        despawnEffect = Effect(15f, 100f) { e: EffectContainer ->
          ...
          Drawf.light(e.x, e.y, 150f, b1, 0.9f * e.fout())
        }
        status = IStatus.熔融
        statusDuration = 300f
        fragBullets = 10
